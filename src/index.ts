...
import { deepAnalysis } from './tools/deep-analysis';
    ...
    return {
      tools: [
        ...
        },
        {
          name: 'deep_analysis',
          description:
            'Perform deep, extended-thinking analysis of code, architecture, or implementation plans. Uses reasoning tokens for thorough step-by-step analysis grounded in the anti-pattern database. Use for complex tasks requiring careful multi-angle evaluation.',
          inputSchema: {
            type: 'object' as const,
            properties: {
              task: {
                type: 'string',
                description: 'What to analyze — describe the problem, question, or code to review.',
              },
              context: {
                type: 'string',
                description: 'Optional code snippets, architecture diagrams (as text), or additional context.',
              },
              tech_stack: {
                type: 'array',
                items: { type: 'string' },
                description: 'Tech stack for filtering relevant anti-patterns.',
              },
              depth: {
                type: 'string',
                enum: ['standard', 'deep', 'exhaustive'],
                description: 'Analysis depth: standard (fast), deep (more reasoning), exhaustive (maximum thinking budget).',
              },
            },
            required: ['task'],
          },
    ...
    try {
      ...
      switch (name) {
        ...
        case 'interview_developer':
          ...
          break;
        case 'deep_analysis':
          result = await deepAnalysis(args as any);
          break;
        default:
